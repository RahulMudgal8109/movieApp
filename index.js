...
async function fetchMovies(page = 1) {
  ...
  if (data.Response == "False") {
    ...
    Search.forEach((movie) => {
      ...
      <h1 class="text-wrap my-1">${i}. ${movie.Title}</h1>
      <button
...
function displayPagination(currentPage, totalResults) {
  ...
  } class="bg-red-500 hover:bg-yellow-600 text-white font-bold py-1 px-4 rounded focus:outline-none focus:shadow-outline">Previous</button>
  <button class="text-2xl font-bold mx-5">${currentPage}</button>
  <button onclick="fetchMovies(${currentPage + 1})" ${
    currentPage == totalResults ? "disabled" : ""
  } class="bg-red-500 hover:bg-yellow-600 text-white font-bold py-1 px-4 rounded focus:outline-none focus:shadow-outline">Next</button>`;
}
